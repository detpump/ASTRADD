-- ============================================================================
-- Migration 001: Add V3 Database Schema
-- 
-- This migration adds all new tables for the V3 data architecture:
...
-- - Layer 3 (Operational): positions (new schema with position_uuid), position_entries, position_closes, bracket_orders
-- - Layer 4 (Risk): risk_states, risk_events, system_components, sync_metrics
-- - Additional: sync_batches, funding_rates
--
-- Status: Can be rolled back using 001_add_v3_schema_rollback.sql
-- ============================================================================
...
-- Mark this migration as pending (will be marked applied after successful execution)
INSERT OR IGNORE INTO schema_migrations (version, name, rollback_sql) 
VALUES (1, '001_add_v3_schema', 'DROP TABLE IF EXISTS schema_migrations;');
...
CREATE INDEX IF NOT EXISTS idx_raw_position_batch ON raw_position_snapshots(sync_batch_id);
CREATE INDEX IF NOT EXISTS idx_raw_position_symbol_time ON raw_position_snapshots(symbol, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_raw_order_batch ON raw_order_snapshots(sync_batch_id);
CREATE INDEX IF NOT EXISTS idx_raw_order_symbol_time ON raw_order_snapshots(symbol, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_raw_order_order_id ON raw_order_snapshots(order_id);
...
CREATE INDEX IF NOT EXISTS idx_raw_account_batch ON raw_account_snapshots(sync_batch_id);
CREATE INDEX IF NOT EXISTS idx_raw_account_asset_time ON raw_account_snapshots(asset, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_events_symbol ON events(symbol, created_at);
CREATE INDEX IF NOT EXISTS idx_events_parent ON events(parent_event_id);
CREATE INDEX IF NOT EXISTS idx_events_caused_by ON events(caused_by_event_id);
...
CREATE INDEX IF NOT EXISTS idx_bracket_state_position ON bracket_state_log(position_uuid);
CREATE INDEX IF NOT EXISTS idx_bracket_state_time ON bracket_state_log(timestamp DESC);
...
CREATE INDEX IF NOT EXISTS idx_scale_in_log_position ON scale_in_conditions_log(position_uuid, timestamp DESC);
CREATE INDEX IF NOT EXISTS idx_scale_in_log_symbol ON scale_in_conditions_log(symbol, timestamp DESC);
...
CREATE INDEX IF NOT EXISTS idx_event_errors_status ON event_processing_errors(status, retry_count);
CREATE INDEX IF NOT EXISTS idx_event_errors_event ON event_processing_errors(event_id);
...
-- Main positions table (revised schema with position_uuid as primary key)
-- NOTE: This replaces the old positions table which used symbol as primary key
-- For backward compatibility, we'll create a new table and later migrate data
...
CREATE INDEX IF NOT EXISTS idx_positions_v3_symbol ON positions_v3(symbol);
CREATE INDEX IF NOT EXISTS idx_positions_v3_status ON positions_v3(status);
CREATE INDEX IF NOT EXISTS idx_positions_v3_cycle ON positions_v3(cycle_id);
CREATE INDEX IF NOT EXISTS idx_positions_v3_correlation ON positions_v3(correlation_id);
CREATE INDEX IF NOT EXISTS idx_positions_v3_scale_pending ON positions_v3(scale_in_pending) WHERE scale_in_pending = 1;
...
CREATE INDEX IF NOT EXISTS idx_position_entries_position ON position_entries(position_uuid, timestamp DESC);
...
CREATE INDEX IF NOT EXISTS idx_position_closes_position ON position_closes(position_uuid, timestamp DESC);
...
CREATE INDEX IF NOT EXISTS idx_bracket_orders_position ON bracket_orders(position_uuid);
CREATE INDEX IF NOT EXISTS idx_bracket_orders_type ON bracket_orders(bracket_type, status);
CREATE INDEX IF NOT EXISTS idx_bracket_orders_status ON bracket_orders(status);
...
CREATE INDEX IF NOT EXISTS idx_risk_events_type_time ON risk_events(event_type, triggered_at DESC);
CREATE INDEX IF NOT EXISTS idx_risk_events_symbol ON risk_events(symbol, triggered_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_sync_metrics_time ON sync_metrics(started_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_sync_batches_status ON sync_batches(status, started_at);
CREATE INDEX IF NOT EXISTS idx_sync_batches_completed ON sync_batches(completed_at) WHERE completed_at IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_funding_rates_symbol_time ON funding_rates(symbol, funding_time DESC);
...
UPDATE schema_migrations SET applied_at = (strftime('%s','now') * 1000) WHERE version = 1;
